exports.views = {
    ...
    files: {
        map: function(doc) {
            if (doc.type && doc.type == 'openerp_log')  emit(doc.ctime, { 
        ...
        map: function(doc) {
            if (doc.type && doc.type == 'openerp_log')  {
                ...
                parser.parse_log(doc.data, function(data) {
                    for(var i = 0; i < data.sql_queries.length; i++) {
                        var query = data.sql_queries[i];
                        emit([ doc.filename, data.request_start, data.request, query ], null);
                    }
                });
            }
        },
    ...
    message_types: {
        ...
        reduce: function(keys, values, rereduce) {
            if(rereduce) {
                ...
                for(var i = 0; i < values.length; i++) {
                    var v = values[i];
                    res.response_time += Number(v.response_time.replace(/s/, ''));
                    res.count++;
    ...
    duplicates: {
        map: function(doc) { 
            if (doc.type && doc.type == 'openerp_log')  {
                var parser = require('views/lib/parser');
                parser.parse_log(doc.data, function(data) {
                    if(data.request && data.response_time) {
